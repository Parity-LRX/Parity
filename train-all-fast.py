...
class EmbedNet(nn.Module):
    ...
    def forward(self, R):
        ...
        # 计算 J
        # 将 A 和 Z 展平为 (batch_size * num_nodes, feature_dim) 和 (batch_size * num_nodes, 3)
        A_flat = A.view(-1, A.shape[-1])  # (batch_size * num_nodes, feature_dim)
        Z_flat = Z.view(-1, 3)  # (batch_size * num_nodes, 3)
...
def compute_E(R, embed_value):
    ...
    embed_net = {
        ...
        6: embed_net1,
        7: embed_net1,
        8: embed_net1}.get(embed_value, embed_net1)
    """
    main_net = {
        0: main_net0,
        1: main_net1,
