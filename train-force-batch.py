...
energy_shift_value = 50
energy_shift_value2 = 0
force_shift_value = 1000
...
class EmbedNet(nn.Module):
    ...
    def forward(self, R):
        ...
        for layer in self.encoder_layers:
            ...
            #S = layer(QS, KS, VS)
        # 将三个矩阵（Z, A, G）连接起来
        S_attention = self.calculate_attention(QS, KS,VS, H, embed_size=embed_size, num_heads=num_heads, num_layers=num_layers)
...
for epoch in range(1, epoch_numbers + 1):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        for input_tensor, read_tensor, target_energy in zip(input_tensors, read_tensors, target_energies):
            ...
            #print(f"Total E_sum for this sample: {total_E_sum * energy_shift_value}")
        ...
        force_loss = (
            ...
            criterion(fz_pred_all, fz_ref.detach().to(device).view(-1))) / (3*force_shift_value)
        ...
        total_loss = ((a * energy_loss + b * force_loss) * energy_shift_value) 
        total_loss.backward()
        ...
        print(f"Epoch {epoch}, Batch {batch_idx + 1}/{len(train_loader)}, "
              f"Energy Loss: {batch_energy_loss / batch_size}, Force Loss: {batch_force_loss / batch_size}, "
              f"Learning Rate: {current_lr1[0]:.6f}",f"batch time: {end_time_batch - start_time_batch:.2f} seconds")
    ...
    for input_tensor, read_tensor, target_energy in val_blocks:  # 使用预加载的数据
        ...
        force_loss_val = (
            ...
            criterion(fz_pred_all_val, fz_ref_val)) / (3*force_shift_value)
