...
emb_number = [100,100,100] #嵌入网络e3MLP最好和主网络e3MLP隐藏层大小一致，层数多一层
...
irreps_query_main = o3.Irreps("50x0e + 50x1o")
irreps_key_main = o3.Irreps("5x0e")
hidden_dim_sh = o3.Irreps("10x0e")
emb_number_main = [64,64]
emb_number_main_2 = [100,100,100]
number_of_basis_main = 16
max_radius_main = 16
...
class embE3Conv(nn.Module):
    ...
    def forward(self, f_in, pos):
        ...
        # 计算球谐函数
        sh = o3.spherical_harmonics(self.irreps_sh_conv, edge_vec, normalize=True, normalization='component').to(device)
...
for epoch in range(1, epoch_numbers + 1):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        with torch.amp.autocast('cuda'):
            ...
            for input_tensor, read_tensor, target_energy in zip(input_tensors, read_tensors, target_energies):
                ...
                E_offset = mapped_A.mean() - energy_mean
                ...
                force_losses.append(force_loss.item())  # 将当前batch的force_loss添加到列表中
                force_rmses.append(force_rmse.item())

            batch_force_loss = torch.mean(torch.tensor(force_losses))
            batch_force_rmse = torch.mean(torch.tensor(force_rmses))
            print(f"Batch force loss: {batch_force_loss.item()}")
            # 计算 Mollifier 正则化项
            ...
            total_force_loss = batch_force_loss + lambda_reg * reg_loss
            ...
            print(f"Epoch {epoch}, Batch {batch_idx + 1}/{len(train_loader)}, "
                f"Total Loss: {total_loss}, Energy RMSE:{energy_rmse}, Force Loss: {total_force_loss}, Force RMSE:{batch_force_rmse}, "
    ...
    for input_tensor, read_tensor, target_energy in val_data:  # 使用预加载的数据
        ...
        E_offset_val = mapped_A_val.mean() - energy_mean

        R_values_val = compute_R(input_tensor)
        ...
        force_loss_val = ((
            criterion(fx_pred_conv_batch_val, fx_ref_val) +
            criterion(fx_pred_conv_batch_val, fy_ref_val) +
            criterion(fx_pred_conv_batch_val, fz_ref_val)) / 3)
        force_losses_val.append(force_loss_val.item())
        force_rmses_val.append(force_rmse_val.item())
    ...
    energy_rmse_val = val_dataset.restore_force(criterion_2(E_sum_val_tensor, (target_E_all_val_tensor)))
    total_force_rmse_val = torch.mean(torch.tensor(force_rmses_val))
    total_force_rmse_val = total_force_rmse_val.item()
    total_energy_rmse_val = energy_rmse_val.item()

    print(f"""Epoch {epoch}/{epoch_numbers},
        Energy RMSE_val: {total_energy_rmse_val},
        Force RMSE_val: {total_force_rmse_val},
    ...
    loss_out.append({
            ...
            "energy_rmse_val": total_energy_rmse_val,
            "force_rmse_val": total_force_rmse_val,
    ...
    if epoch % 1 == 0:
        ...
        loss_out_df.to_csv(f'epoch_{epoch}_batch_count_{batch_count}_loss.csv', index=False)
                # 早停机制
    if total_energy_rmse_val < best_val_loss:
        best_val_loss = total_energy_rmse_val
        patience_counter = 0  
    else:
        patience_counter += 1 
